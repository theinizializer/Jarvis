...
Logica:
  ...
  - Configura segreti cifrati (sudo password + PIN) via SecretsManager/Fernet
...
def move_and_patch_files():
    ...
    # Lista completa di tutti i moduli del progetto v9.
    files = [
        "jarvis_v9.py",            # orchestratore principale (era jarvis_v8.py — corretto)
        "api_module.py",           # provider LLM (Groq/Cerebras/NVIDIA/Ollama)
        "discord_module.py",       # integrazione Discord (mancava → "non trovato")
        "utils_module.py",         # costanti, regex, run_cmd, CleanupManager
        "commands_module.py",      # comandi slash (/tts, /setpin, /dormi, ...)
        "jarvis_memory_engine.py", # memoria SQLite + ChromaDB
        "voice_module.py",         # STT/TTS/wake word/speaker verification
        "search_module.py",        # ricerca web
        "language_module.py",      # localizzazione multilingua
        "agent_module.py",         # pianificazione autonoma
        "blender_module.py",       # integrazione Blender (bpy)
        "sepformer_server.py",     # server speaker extraction (socket Unix)
        "jarvis_banner.py",        # TUI / banner
        "jarvis_secrets.py",       # gestione segreti cifrati (Fernet)
        "ssh_module.py",           # SSH multi-host
        "persona_module.py",       # personalità/persona di JARVIS
        "email_module.py",         # integrazione Gmail (IMAP/SMTP)
        "user_registry.py",        # registro identità (nome -> ruolo -> pin_hash)
        "pin_auth.py",             # hashing/verifica PIN (pbkdf2)
        "command_policy.py",       # policy comandi del guardiano (allow/deny/confirm)
    ]
    ...
    # Avviso se manca il file principale
    if "jarvis_v9.py" not in copied:
        err("jarvis_v9.py NON copiato — JARVIS non potrà avviarsi!")
...
def setup_env():
    step(7, 8, "API Key (.env) + segreti cifrati")
    ...
    lines = ["# JARVIS v9.0 — Configurazione\n# Non committare questo file!\n"]
    for key, meta in keys.items():
        lines.append(f"# {meta['desc']}")
        lines.append(f"{key}={new_values.get(key, '')}\n")
    # Sorgenti configurabili (PC = localhost, Raspberry = IP del PC)
    lines.append("# ── Sorgenti (PC=localhost, Raspberry=IP del PC) ──")
    lines.append("OLLAMA_HOST=localhost")
    lines.append("OLLAMA_PORT=11434")
    lines.append("# Modello Whisper: medium (PC) | base | tiny (Raspberry, piu' veloce)")
    lines.append("WHISPER_MODEL=medium\n")
    ...
    # ── Segreti cifrati: sudo password + PIN di sessione ──────────────────────
    # Usa il SecretsManager (Fernet) per i dati sensibili, che NON devono stare
    # in chiaro nel .env: password sudo e PIN di sessione.
    _setup_encrypted_secrets()


def _setup_encrypted_secrets():
    """
    Configura i segreti cifrati (sudo password + PIN) tramite SecretsManager.
    Importa jarvis_secrets dalla cartella di destinazione (appena copiato).
    """
    secrets_file = DEST_DIR / "jarvis_secrets.py"
    if not secrets_file.exists():
        warn("jarvis_secrets.py non trovato — salto sudo/PIN cifrati")
        return

    print()
    bold("   ─── Segreti cifrati (sudo + PIN) ───────────────────────")
    print("   Questi dati sensibili vengono cifrati con Fernet e legati")
    print("   a questa macchina — non finiscono mai in chiaro nel .env.")
    print()

    if not confirm("   Configurare ora sudo password e/o PIN cifrati?"):
        info("   Saltato — potrai configurarli dopo con il comando /setpin in JARVIS")
        return

    # Importa il SecretsManager dalla cartella di destinazione
    sys.path.insert(0, str(DEST_DIR))
    try:
        from jarvis_secrets import SecretsManager
    except Exception as e:
        warn(f"Impossibile importare SecretsManager: {e}")
        return
    ...
    if not sm.crypto_available:
        warn("cryptography non installato — i segreti NON saranno cifrati")

    import getpass

    # ── Password sudo ─────────────────────────────────────────────────────────
    print()
    print("   Password sudo (per comandi di sistema — invio per saltare):")
    try:
        sudo = getpass.getpass("   sudo > ")
    except Exception:
        sudo = input("   sudo > ").strip()
    if sudo:
        new_values["SUDO_PASSWORD"] = sudo
        ok("   Password sudo configurata (cifrata)")

    # ── Gmail (App Password) ─────────────────────────────────────────────────
    print()
    print("   Gmail — per leggere/inviare email (invio per saltare):")
    print("   Serve una 'App Password': Google Account → Sicurezza →")
    print("   Verifica in 2 passaggi → Password per le app")
    gmail_addr = input("   Indirizzo Gmail > ").strip()
    if gmail_addr:
        try:
            gmail_pwd = getpass.getpass("   App Password > ").strip()
        except Exception:
            gmail_pwd = input("   App Password > ").strip()
        if gmail_pwd:
            new_values["GMAIL_ADDRESS"] = gmail_addr
            new_values["GMAIL_APP_PASSWORD"] = gmail_pwd
            ok("   Gmail configurato (cifrato)")

    # ── PIN di sessione + ADMIN ───────────────────────────────────────────────
    print()
    print("   Nome dell'amministratore (sei tu — JARVIS ti riconoscerà con questo):")
    admin_name = input("   Nome admin > ").strip().lower()
    print()
    print("   PIN di sessione (protegge JARVIS al terminale):")
    print("   [1] PIN numerico   [2] Password   (invio = nessun PIN)")
    pin_choice = input("   > ").strip()
    if pin_choice in ("1", "2"):
        pin_type = "pin" if pin_choice == "1" else "password"
        try:
        ...
        if pin1 and pin1 == pin2:
            if pin_type == "pin" and not pin1.isdigit():
                warn("   PIN numerico deve contenere solo cifre — saltato")
            else:
                new_values["JARVIS_PIN"] = pin1
                new_values["JARVIS_PIN_TYPE"] = pin_type
                ok(f"   PIN configurato ({pin_type})")
                # Crea l'admin nel REGISTRO: PIN salvato come HASH in users.json,
                # non in chiaro. È questo che impedisce la duplicazione dell'admin
                # (un secondo init_admin viene rifiutato).
                if admin_name:
                    try:
                        import sys as _sys
                        _sys.path.insert(0, str(CODE_DIR))
                        from user_registry import UserRegistry
                        _okr, _info = UserRegistry(DEST_DIR / "users.json").init_admin(admin_name, pin1)
                        if _okr:
                            ok(f"   Admin '{_info}' creato nel registro (PIN hashato)")
                        else:
                            warn(f"   Registro admin: {_info}")
                    except Exception as _e:
                        warn(f"   Registro non creato ({_e}) — riprova al primo avvio")
                else:
                    warn("   Nome admin vuoto — registro non creato, JARVIS userà il PIN in chiaro")
        else:
            warn("   PIN non corrispondente o vuoto — saltato")

    # ── Salva cifrato ─────────────────────────────────────────────────────────
    if new_values != existing:
        try:
            sm.save(new_values)
            ok(f"Segreti cifrati salvati in {enc_path}")
            if sm.crypto_available:
                info("File cifrato — inutilizzabile su altre macchine")
        except Exception as e:
            warn(f"Errore salvataggio segreti: {e}")
    else:
        info("Nessun segreto cifrato configurato")
...
{C.BOLD}Sicurezza:{C.RESET}
   • Password sudo e PIN sono cifrati (Fernet) in .env.enc
   • Cambia il PIN in qualsiasi momento con il comando {C.BLUE}/setpin{C.RESET}
   • La password sudo non viene più mostrata a schermo
