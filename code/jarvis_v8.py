...
class Jarvis:
    ...
    # ── Prompt dinamico ───────────────────────────────────────────────────────
    def _sys_prompt(self, query: str = "") -> str:
    ...
    def _call_nvidia(self, messages: list, tools: list,
        ...
        headers = {
            ...
            "Accept":        "text/event-stream",  # formato ufficiale NVIDIA
            "Content-Type":  "application/json",
        }

        # Costruisci messaggi — se c'è immagine, aggiungila all'ultimo user message
        msgs = list(messages)
        if image_b64:
        ...
        payload = {
            ...
            "stream":             True,
            # Attiva il ragionamento profondo di Qwen
            "chat_template_kwargs": {"enable_thinking": True},
        }
        # Tool calling solo se non c'è immagine (vision e tools non si mischiano bene)
        if tools and not image_b64:
            payload["tools"]       = tools
            payload["tool_choice"] = "auto"
        full_text   = ""
        think_text  = ""   # buffer pensieri <think>...</think>
        tool_calls  = []
        tts_buf     = ""
        in_think    = False  # siamo dentro <think>?
        _tc_buf: dict[int, dict] = {}
        ...
        with resp_nvidia as resp:
            for line in resp.iter_lines():
                ...
                chunk  = delta.get("content") or ""
                if chunk:
                    # Gestisci tag <think>...</think> — mostrati ma non letti ad alta voce
                    if "<think>" in chunk:
                        in_think = True
                    ...
                    if in_think:
                        think_text += chunk
                        print(chunk, end="", flush=True)  # mostra i pensieri in grigio
                        continue
                    # Testo normale — stampa e TTS
                    full_text += chunk
                    tts_buf   += chunk
                    print(chunk, end="", flush=True)
                    if any(c in chunk for c in ('.', '!', '?', '\n')):
    ...
    def _call_model(self, user_msg, with_image=False, history=None, web_context=""):
        ...
        sys_content = self._sys_prompt(query=user_msg)
        if web_context:
            sys_content += (
