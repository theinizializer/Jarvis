...
# ── Hashing dei PIN ────────────────────────────────────────────────────────────
# I PIN si HASHANO, non si cifrano: servono solo a verificare, mai a rileggere.
# Cosi' anche decifrando .env.enc non si recupera il PIN. PBKDF2-SHA256, 200k iter.
_PIN_ITER = 200_000

def _hash_secret(value: str, salt: bytes = None) -> str:
    salt = salt or os.urandom(16)
    h = hashlib.pbkdf2_hmac("sha256", value.encode("utf-8"), salt, _PIN_ITER)
    return f"{salt.hex()}${h.hex()}"

def _verify_secret(value: str, stored: str) -> bool:
    try:
        salt_hex, hash_hex = stored.split("$", 1)
        h = hashlib.pbkdf2_hmac("sha256", value.encode("utf-8"),
                                bytes.fromhex(salt_hex), _PIN_ITER)
...
class SecretsManager:
    ...
    def save(self, data: dict):
        ...
        # NB: NON cancelliamo il .env in chiaro!
        # .env (API key) e .env.enc (sudo/PIN) sono file separati con scopi diversi.
        # Prima questo unlink() cancellava il .env dell'utente ogni volta che si
        # salvava un segreto (es. /setpin), facendo sparire CEREBRAS_API_KEY →
        # Cerebras non disponibile → JARVIS cadeva su Ollama. Bug risolto.
    ...
    # ── PIN e utenti ──────────────────────────────────────────────────────────
    #  Ogni utente = nome + ruolo ('admin'|'secondary') + pin_hash (+ tipo, lingua).
    #  Salvati come JSON su una riga nella chiave JARVIS_USERS.

    def _users(self) -> dict:
        raw = self.get("JARVIS_USERS", "")
        if not raw:
            return {}
        try:
    ...
    def _save_users(self, users: dict):
        self.set("JARVIS_USERS", json.dumps(users, separators=(",", ":")))

    def _absorb_legacy_pin(self):
        """Se c'e' un JARVIS_PIN in chiaro (vecchio formato, o scritto da /setpin),
        lo trasforma in hash sull'utente admin e rimuove il chiaro. Mantiene
        funzionante il vecchio flusso: lo STESSO PIN resta valido, nessuno resta
        chiuso fuori. Costo trascurabile quando il PIN in chiaro non c'e'."""
        if not self._data:
            self.load()
        legacy = self._data.get("JARVIS_PIN", "")
        if not legacy:
            return
        ptype = self._data.get("JARVIS_PIN_TYPE", "pin")
        users = self._users()
        admin_name = next((n for n, u in users.items() if u.get("role") == "admin"), "admin")
        users[admin_name] = {"role": "admin",
                             "pin_hash": _hash_secret(legacy.strip()),
                             "type": ptype}
        self._data.pop("JARVIS_PIN", None)
        self._data.pop("JARVIS_PIN_TYPE", None)
        self._data["JARVIS_USERS"] = json.dumps(users, separators=(",", ":"))
        self.save(self._data)
        print("🔒 PIN salvato come hash (lo stesso PIN resta valido)", file=sys.stderr)

    def add_user(self, name: str, pin: str, role: str = "secondary",
                 pin_type: str = "pin", lang: str = "") -> bool:
        """Crea/aggiorna un utente con PIN hashato. role: 'admin' | 'secondary'."""
        name = (name or "").strip().lower()
        if not name or not pin:
            return False
        users = self._users()
        entry = {"role": role, "pin_hash": _hash_secret(pin.strip()), "type": pin_type}
        if lang:
            entry["lang"] = lang
        users[name] = entry
    ...
    def set_admin(self, name: str, pin: str, pin_type: str = "pin", lang: str = "") -> bool:
        return self.add_user(name, pin, role="admin", pin_type=pin_type, lang=lang)

    def remove_user(self, name: str) -> bool:
        users = self._users()
        if (name or "").strip().lower() in users:
            del users[name.strip().lower()]
            self._save_users(users)
            return True
        return False

    def get_role(self, name: str) -> str:
        u = self._users().get((name or "").strip().lower())
        return u.get("role", "") if u else ""

    def list_users(self) -> dict:
        return {n: u.get("role", "secondary") for n, u in self._users().items()}

    def identify_pin(self, entered: str):
        """Ritorna (name, role) del primo utente con PIN combaciante, o None."""
        entered = (entered or "").strip()
        if not entered:
            return None
        for name, u in self._users().items():
            if _verify_secret(entered, u.get("pin_hash", "")):
                return name, u.get("role", "secondary")
        return None

    def verify_pin(self, entered: str) -> bool:
        """Sblocco di sessione: True se il PIN combacia con un utente.
        Salva last_user/last_role (chi ha sbloccato) per chi chiama.
        Nessun utente configurato -> accesso libero (come prima)."""
        self._absorb_legacy_pin()
        if not self._users():
            return True
        res = self.identify_pin(entered)
        if res:
            self.last_user, self.last_role = res
            return True
        return False
    ...
    if pin_type_choice in ("1", "2", "3"):
        ...
        if pin1 == pin2 and pin1:
            if pin_type == "pin" and not pin1.isdigit():
                ...
                _admin_name = ((input("  Nome admin (invio = utente di sistema): ").strip().lower())
                               or (os.getenv("USER") or "admin"))
                new_values["JARVIS_USERS"] = json.dumps(
                    {_admin_name: {"role": "admin",
                                   "pin_hash": _hash_secret(pin1.strip()),
                                   "type": pin_type}},
                    separators=(",", ":"))
                new_values.pop("JARVIS_PIN", None)
                print(f"  ✅ Admin '{_admin_name}' configurato — PIN hashato ({pin_type})")
        else:
