...
class SSHManager:
    ...
    def __init__(self, config_path: str = "~/.jarvis/ssh_hosts.json"):
        ...
        # Fernet legato alla macchina, riusato dal SecretsManager, per cifrare
        # le password SSH a riposo nel JSON (prima erano in chiaro).
        self._fernet = self._build_fernet()
        self._load()

    @staticmethod
    def _build_fernet():
        """Costruisce la stessa chiave Fernet del SecretsManager (machine-bound)."""
        try:
            from jarvis_secrets import SecretsManager
            sm = SecretsManager()
            return getattr(sm, "_fernet", None)
        except Exception:
            return None

    def _enc(self, plain: str) -> str:
        """Cifra una password. Prefisso 'enc:' per distinguerla dal testo in chiaro."""
        if not plain or self._fernet is None:
            return plain
        try:
    ...
    def _dec(self, stored: str) -> str:
        """Decifra una password se ha il prefisso 'enc:'; altrimenti la ritorna così."""
        if not stored or not stored.startswith("enc:") or self._fernet is None:
            return stored
        try:
    ...
    def _load(self):
        if self._config_path.exists():
            try:
                ...
                for h in data:
                    # Decifra la password al caricamento
                    if h.get("password"):
                        h["password"] = self._dec(h["password"])
                    rh = RemoteHost(h)
    ...
    def _save(self):
        ...
        for h in self._hosts.values():
            d = h.to_dict()
            # Cifra la password prima di scrivere su disco
            if d.get("password"):
                d["password"] = self._enc(d["password"])
            data.append(d)
        self._config_path.write_text(
            json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8"
        )
        # Permessi restrittivi sul file (solo proprietario)
        try:
            import os as _os, platform as _pf
            if _pf.system() != "Windows":
                _os.chmod(self._config_path, 0o600)
        except Exception:
            pass
