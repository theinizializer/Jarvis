...
# ══════════════════════════════════════════════════════════════════════════════
# MESSAGGI COMUNI — quelli che l'utente vede davvero (menu, conferme, errori)
# ══════════════════════════════════════════════════════════════════════════════
# Tradotti per le lingue effettivamente usate: it, fr, en, pt.
# Per le altre lingue, fallback automatico all'inglese (en), poi alla chiave.
# Struttura: COMMON_MESSAGES[chiave][lingua] = testo (può avere {placeholder}).
COMMON_MESSAGES = {
    # ── PIN / sessione ──
    "pin_locked": {
        "it": "🔐 Sessione bloccata — inserisci PIN per iniziare",
        "fr": "🔐 Session verrouillée — entrez le PIN pour commencer",
        "en": "🔐 Session locked — enter PIN to start",
        "pt": "🔐 Sessão bloqueada — introduza o PIN para começar",
    },
    "pin_wrong": {
        "it": "❌ PIN errato",
        "fr": "❌ PIN incorrect",
    ...
    "pin_ok": {
        "it": "ok Accesso consentito",
        "fr": "ok Accès autorisé",
        "en": "ok Access granted",
        "pt": "ok Acesso permitido",
    },
    "pin_too_many": {
        "it": "❌ Troppi tentativi — uscita",
        "fr": "❌ Trop de tentatives — sortie",
        "en": "❌ Too many attempts — exiting",
        "pt": "❌ Demasiadas tentativas — a sair",
    },
    "pin_set_ok": {
        "it": "✅ PIN configurato ({type}). Richiesto al prossimo avvio in modalità tastiera.",
        "fr": "✅ PIN configuré ({type}). Demandé au prochain démarrage en mode clavier.",
        "en": "✅ PIN configured ({type}). Requested on next startup in keyboard mode.",
        "pt": "✅ PIN configurado ({type}). Pedido no próximo arranque em modo teclado.",
    },
    "pin_mismatch": {
        "it": "❌ I due PIN non corrispondono — annullato",
        "fr": "❌ Les deux PIN ne correspondent pas — annulé",
        "en": "❌ The two PINs don't match — cancelled",
        "pt": "❌ Os dois PINs não correspondem — cancelado",
    },
    # ── Comandi ──
    "cmd_not_found": {
        "it": "❌ Comando non trovato: {cmd}\n   Digita /aiuto per la lista comandi",
        "fr": "❌ Commande introuvable : {cmd}\n   Tapez /aide pour la liste des commandes",
        "en": "❌ Command not found: {cmd}\n   Type /help for the command list",
        "pt": "❌ Comando não encontrado: {cmd}\n   Escreva /ajuda para a lista de comandos",
    },
    "commands_available": {
    ...
    "needs_arg": {
        "it": "ℹ️  {cmd} richiede un argomento — digitalo direttamente.",
        "fr": "ℹ️  {cmd} nécessite un argument — tapez-le directement.",
        "en": "ℹ️  {cmd} requires an argument — type it directly.",
        "pt": "ℹ️  {cmd} requer um argumento — escreva-o diretamente.",
    },
    # ── Modalità input ──
    "mode_keyboard": {
        "it": "⌨️  Passato a modalità tastiera.",
        "fr": "⌨️  Passé en mode clavier.",
        "en": "⌨️  Switched to keyboard mode.",
        "pt": "⌨️  Mudou para modo teclado.",
    },
    "mode_voice": {
        "it": "🎙️  Passato a modalità vocale.",
        "fr": "🎙️  Passé en mode vocal.",
        "en": "🎙️  Switched to voice mode.",
        "pt": "🎙️  Mudou para modo voz.",
    },
    "mic_fail": {
        "it": "❌ Impossibile attivare il microfono.",
        "fr": "❌ Impossible d'activer le microphone.",
        "en": "❌ Could not activate the microphone.",
        "pt": "❌ Não foi possível ativar o microfone.",
    },
    # ── TTS ──
    "tts_on": {
        "it": "🔊 TTS ON ✅",
    ...
    "voice_unavailable": {
        "it": "❌ Modalità vocale non disponibile",
        "fr": "❌ Mode vocal non disponible",
        "en": "❌ Voice mode unavailable",
        "pt": "❌ Modo voz indisponível",
    },
    "voice_back_keyboard": {
        "it": "   Torno alla modalità tastiera.",
        "fr": "   Retour au mode clavier.",
        "en": "   Returning to keyboard mode.",
        "pt": "   A voltar ao modo teclado.",
    },
    # ── Generici ──
    "goodbye": {
        "it": "👋 Ciao!",
    ...
    # ── Descrizioni comandi (per /aiuto) ──
    "d_lingua":          {"it": "Lingua corrente", "fr": "Langue actuelle", "en": "Current language", "pt": "Idioma atual"},
    "d_cambia_lingua":   {"it": "Cambia lingua attiva", "fr": "Changer de langue", "en": "Change active language", "pt": "Mudar idioma ativo"},
    "d_aggiungi_lingua": {"it": "Aggiungi nuova lingua", "fr": "Ajouter une langue", "en": "Add new language", "pt": "Adicionar idioma"},
    "d_rimuovi_lingua":  {"it": "Rimuovi una lingua", "fr": "Supprimer une langue", "en": "Remove a language", "pt": "Remover idioma"},
    "d_memorizza":       {"it": "Salva un fatto: /memorizza <testo>", "fr": "Enregistrer un fait : /memorizza <texte>", "en": "Save a fact: /memorizza <text>", "pt": "Guardar um facto: /memorizza <texto>"},
    "d_memoria":         {"it": "Mostra fatti memorizzati", "fr": "Afficher les faits mémorisés", "en": "Show stored facts", "pt": "Mostrar factos guardados"},
    "d_dimentica":       {"it": "Cancella memoria", "fr": "Effacer la mémoire", "en": "Clear memory", "pt": "Limpar memória"},
    "d_tts":             {"it": "Attiva/disattiva voce TTS", "fr": "Activer/désactiver la voix TTS", "en": "Toggle TTS voice", "pt": "Ativar/desativar voz TTS"},
    "d_aggiungi_voce":   {"it": "Aggiungi profilo vocale", "fr": "Ajouter un profil vocal", "en": "Add voice profile", "pt": "Adicionar perfil de voz"},
    "d_profili_voce":    {"it": "Gestisci profili vocali", "fr": "Gérer les profils vocaux", "en": "Manage voice profiles", "pt": "Gerir perfis de voz"},
    "d_modalita":        {"it": "Cambia modalita input (tastiera/voce)", "fr": "Changer le mode d'entrée (clavier/voix)", "en": "Change input mode (keyboard/voice)", "pt": "Mudar modo de entrada (teclado/voz)"},
    "d_meteo":           {"it": "Meteo: /meteo <citta>", "fr": "Météo : /meteo <ville>", "en": "Weather: /meteo <city>", "pt": "Tempo: /meteo <cidade>"},
    "d_notizie":         {"it": "Ultime notizie", "fr": "Dernières nouvelles", "en": "Latest news", "pt": "Últimas notícias"},
    "d_wiki":            {"it": "Wikipedia: /wiki <argomento>", "fr": "Wikipédia : /wiki <sujet>", "en": "Wikipedia: /wiki <topic>", "pt": "Wikipédia: /wiki <tópico>"},
    "d_host":            {"it": "Gestione host SSH", "fr": "Gestion des hôtes SSH", "en": "SSH host management", "pt": "Gestão de hosts SSH"},
    "d_agente":          {"it": "Modalita agente: /agente <obiettivo>", "fr": "Mode agent : /agente <objectif>", "en": "Agent mode: /agente <goal>", "pt": "Modo agente: /agente <objetivo>"},
    "d_provider":        {"it": "Cambia modello AI", "fr": "Changer de modèle IA", "en": "Change AI model", "pt": "Mudar modelo de IA"},
    "d_stats":           {"it": "Statistiche sessione", "fr": "Statistiques de session", "en": "Session statistics", "pt": "Estatísticas da sessão"},
    "d_token":           {"it": "Consumo token per provider", "fr": "Consommation de tokens par fournisseur", "en": "Token usage per provider", "pt": "Consumo de tokens por fornecedor"},
    "d_setpin":          {"it": "Imposta o cambia il PIN di sessione", "fr": "Définir ou changer le PIN de session", "en": "Set or change session PIN", "pt": "Definir ou mudar o PIN de sessão"},
    "d_dormi":           {"it": "Standby (blocca con PIN)", "fr": "Veille (verrouiller avec PIN)", "en": "Standby (lock with PIN)", "pt": "Suspender (bloquear com PIN)"},
    "d_esci":            {"it": "Esci da JARVIS", "fr": "Quitter JARVIS", "en": "Exit JARVIS", "pt": "Sair do JARVIS"},
    "quit_label":        {"it": "Esci", "fr": "Quitter", "en": "Quit", "pt": "Sair"},
    "executing":         {"it": "Eseguo", "fr": "Exécution", "en": "Executing", "pt": "A executar"},
    "lang_changed_to":   {"it": "Lingua cambiata:", "fr": "Langue changée :", "en": "Language changed:", "pt": "Idioma alterado:"},
}
...
class LanguageManager:
    ...
    def c(self, key: str, **kwargs) -> str:
        """
        Messaggio comune localizzato (menu, conferme, errori che l'utente vede).
        Fallback: lingua corrente → inglese → italiano → chiave stessa.
        """
        entry = COMMON_MESSAGES.get(key, {})
        msg = entry.get(self.current) or entry.get("en") or entry.get("it") or key
        try:
    ...
    def handle_slash(self, text: str, ask_fn=None, print_fn=None) -> Optional[str]:
        """Processa un comando slash — localizzato.
        ask_fn:   usato per input interattivo invece di input().
        print_fn: usato per output invece di print().
        """
        t   = text.strip()
        cmd = t.lower()
        ...
        _ask   = ask_fn   or (lambda p, ph=None: input("> ").strip())
        ...
        for key, slash in cmds.items():
            if cmd == slash.lower() or cmd.startswith(slash.lower()):
                ...
                if key in ("/cambia_lingua", "/change_language", "/changer_langue",
                           "/sprache_wechseln", "/wiesselen",
                           "/сменить", "/切换语言", "/言語変更", "/언어변경"):
                    return self._cmd_change(ask_fn=_ask, print_fn=_print)
                if key in ("/aggiungi", "/add", "/ajouter", "/hinzufügen", "/derbäisetzen",
                           "/добавить", "/添加语言", "/言語追加", "/언어추가"):
                    return self._cmd_add(rest, ask_fn=_ask, print_fn=_print)
                if key in ("/rimuovi", "/remove", "/supprimer", "/entfernen", "/ewechhuelen",
                           "/удалить", "/删除语言", "/언어삭제"):
                    return self._cmd_remove(rest, ask_fn=_ask, print_fn=_print)
        ...
        # Fallback generico
        if "/cambia_lingua" in cmd or "/change_language" in cmd or "/changer_langue" in cmd or "/sprache_wechsel" in cmd:
            return self._cmd_change(ask_fn=_ask, print_fn=_print)
    ...
    def _cmd_change(self, ask_fn=None, print_fn=None) -> str:
        _ask   = ask_fn   or (lambda p, ph=None: input("> ").strip())
        _print = print_fn or print
        ...
        for i, code in enumerate(langs, 1):
            name   = ALL_LANGUAGES.get(code, code)
            marker = " ◀ attiva" if code == self.current else ""
            _print(f"  {i}. [{code}] {name}{marker}")
        _print("  " + "-" * 30)
        scelta = _ask("  Numero (esc = annulla): ", "numero o esc").strip().lower()
        if not scelta or scelta in ("esc", "exit", "q", "annulla"):
            return "Annullato."
        try:
            idx  = int(scelta) - 1
            code = langs[idx]
    ...
    def _cmd_add(self, query: str, ask_fn=None, print_fn=None) -> str:
        _ask   = ask_fn   or (lambda p, ph=None: input("> ").strip())
        _print = print_fn or print
        code   = self._find_lang(query)
        if not code:
            codes = list(ALL_LANGUAGES.keys())
            _print("\n  Scegli lingua da aggiungere:")
            _print("  " + "-" * 30)
            for i, c in enumerate(codes, 1):
                already = " (gia' presente)" if c in self.my_languages else ""
                _print(f"  {i:2}. [{c}] {ALL_LANGUAGES[c]}{already}")
            _print("  " + "-" * 30)
            scelta = _ask("  Numero (esc = annulla): ", "numero o esc").strip().lower()
            if not scelta or scelta in ("esc", "exit", "q", "annulla"):
                return "Annullato."
    ...
    def _cmd_remove(self, query: str, ask_fn=None, print_fn=None) -> str:
        _ask   = ask_fn   or (lambda p, ph=None: input("> ").strip())
        _print = print_fn or print
        code   = self._find_lang(query)
        if not code:
            langs = self.my_languages
            if len(langs) <= 1:
                return "Non puoi rimuovere l'unica lingua attiva."
            _print("\n  Scegli lingua da rimuovere:")
            _print("  " + "-" * 30)
            for i, c in enumerate(langs, 1):
                name   = ALL_LANGUAGES.get(c, c)
                marker = " ◀ attiva" if c == self.current else ""
                _print(f"  {i}. [{c}] {name}{marker}")
            _print("  " + "-" * 30)
            scelta = _ask("  Numero (esc = annulla): ", "numero o esc").strip().lower()
            if not scelta or scelta in ("esc", "exit", "q", "annulla"):
                return "Annullato."
            try:
                code = langs[int(scelta) - 1]
            except (ValueError, IndexError):
                return self.t("invalid")
