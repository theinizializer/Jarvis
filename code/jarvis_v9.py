...
# ─── JARVIS ───────────────────────────────────────────────────────────────────
# I mixin aggiungono i metodi API e Discord — se i moduli non sono trovati
# i fallback inline presenti nel corpo della classe vengono usati automaticamente.
class Jarvis(JarvisAPIMixin, JarvisDiscordMixin):
...
def main():
    ...
    def _request_pin(prompt="🔐 PIN: ") -> bool:
        ...
        if not _secrets_mgr.verify_pin(entered):
            _chat_print("   " + (lang_mgr.c("pin_wrong") if lang_mgr else "❌ PIN errato")); return False
        return True
