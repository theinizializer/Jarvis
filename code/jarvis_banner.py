...
def _build_banner_markup(
    ...
    info = [
        ...
        *load_rows,
        ...
        ("Speaker", _ok(speaker_ok)), ("Memoria", f"{memory_count} fatti"),
        ("Token", f"[magenta]{tokens}[/magenta]"), ("Ora", ora),
    ]
    logo_w = 52; lines = []
...
if TEXTUAL_OK:
    ...
    class BannerWidget(Static):
        ...
        def _params(self):
            ...
            return dict(model=bot.model, tts_on=bot.tts_on, discord_on=bot.disc_on,
                        ...
                        tokens=getattr(bot, "token_summary_short", "0"))
    ...
    class JarvisApp(App):
        ...
        def on_input_submitted(self, event: Input.Submitted):
            ...
            # In modalità segreta (PIN/password) NON mostrare il testo nella chat
            # e NON mascherare il valore reale passato al worker.
            if getattr(self, "_secret_mode", False):
                # Disattiva subito la modalità segreta dopo l'invio
                self.set_secret(False)
                self._msg_q.put(("system", "🔐 ••••• (inserito)"))
            else:
        ...
        def set_secret(self, on: bool):
            """Attiva/disattiva la modalità input segreto (maschera i caratteri)."""
            self._secret_mode = on
            if self._ibox:
                # Textual Input.password=True mostra i caratteri come •
                try:
                    self._ibox.password = on
                except Exception:
                    pass
...
def _build_banner_lines_ansi(
    ...
    info=[
        ...
        ("Speaker",_ok_a(speaker_ok)),("Memoria",f"{memory_count} fatti"),
        ("Token",f"{CYAN}{tokens}{RST}"),("Ora",ora),
    ]
    logo_w=52; lines=[]
    for i,ll in enumerate(_LOGO_TOP):
        pad=" "*max(0,logo_w-len(ll)); s=""
